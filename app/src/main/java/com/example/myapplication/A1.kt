...
import com.example.myapplication.databinding.ActivityA1Binding
...
class A1 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.imageViewBackA1.setOnClickListener {
    ...
    private fun performTransaction(amount: Int){
        Firebase.firestore.collection("users").document("0A1_123")
            .get()
            .addOnCompleteListener { task->
                if(task.isSuccessful){
                    var currentAmount = task.result.toObject(User::class.java)?.balance.toString().toInt()
